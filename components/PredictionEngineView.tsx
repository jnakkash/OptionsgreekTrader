    ...
    <div className="w-full max-w-7xl mx-auto mt-8 animate-in fade-in pb-12">
      ...
      {data && !loading && (
        <div className="space-y-8 animate-in slide-in-from-bottom-4 fade-in">
          ...
          {/* Walk-Forward Strategy Performance Table — Sortino / Calmar / MaxDD */}
          {quantData?.strategyBacktests && quantData.strategyBacktests.length > 0 && (
            <div className="bg-[#111] border border-gray-800 rounded-xl p-6 shadow-xl">
              <h3 className="text-lg font-bold text-white flex items-center gap-2 mb-4">
                <Activity className="text-amber-400" size={18} /> Purged Walk-Forward Strategy Performance Matrix
              </h3>
              <div className="overflow-x-auto">
                <table className="w-full text-xs font-mono border-collapse">
                  <thead>
                    <tr className="border-b border-gray-800 bg-black/40 text-gray-400">
                      <th className="py-2.5 px-3 text-left font-bold">Strategy</th>
                      <th className="py-2.5 px-3 text-left font-bold">Signal</th>
                      <th className="py-2.5 px-3 font-bold text-right">Trades</th>
                      <th className="py-2.5 px-3 font-bold text-right">Win Rate</th>
                      <th className="py-2.5 px-3 font-bold text-right">Return %</th>
                      <th className="py-2.5 px-3 font-bold text-right">Prof. Factor</th>
                      <th className="py-2.5 px-3 font-bold text-right text-amber-400">Sortino</th>
                      <th className="py-2.5 px-3 font-bold text-right text-sky-400">Calmar</th>
                      <th className="py-2.5 px-3 font-bold text-right text-rose-400">Max DD %</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-800/40">
                    {quantData.strategyBacktests.map((st, i) => {
                      const signalColor = st.signal === 'BULLISH' ? 'text-emerald-400' : st.signal === 'BEARISH' ? 'text-rose-400' : 'text-gray-400';
                      const retColor = st.totalReturnPercent >= 0 ? 'text-emerald-400' : 'text-rose-400';
                      return (
                        <tr key={i} className="hover:bg-white/5 transition-colors">
                          <td className="py-2.5 px-3 text-white font-semibold">{st.strategyName}</td>
                          <td className={`py-2.5 px-3 font-bold ${signalColor}`}>{st.signal}</td>
                          <td className="py-2.5 px-3 text-right text-gray-300">{st.totalTrades}</td>
                          <td className="py-2.5 px-3 text-right text-white">{st.winRate.toFixed(1)}%
                            <span className="text-gray-500 text-[9px] ml-1">±{((st.winRateCI95.upper - st.winRateCI95.lower)/2).toFixed(1)}</span>
                          </td>
                          <td className={`py-2.5 px-3 text-right font-bold ${retColor}`}>{st.totalReturnPercent >= 0 ? '+' : ''}{st.totalReturnPercent.toFixed(1)}%</td>
                          <td className="py-2.5 px-3 text-right text-gray-300">{st.profitFactor.toFixed(2)}x</td>
                          <td className={`py-2.5 px-3 text-right font-bold ${(st.sortinoRatio ?? 0) >= 1 ? 'text-emerald-400' : (st.sortinoRatio ?? 0) >= 0 ? 'text-amber-400' : 'text-rose-400'}`}>
                            {st.totalTrades < 5 ? '—' : (st.sortinoRatio ?? 0).toFixed(2)}
                          </td>
                          <td className={`py-2.5 px-3 text-right font-bold ${(st.calmarRatio ?? 0) >= 1 ? 'text-sky-400' : (st.calmarRatio ?? 0) >= 0 ? 'text-amber-400' : 'text-rose-400'}`}>
                            {(st.calmarRatio ?? 0).toFixed(2)}
                          </td>
                          <td className="py-2.5 px-3 text-right text-rose-400 font-bold">
                            {(st.maxDrawdownPct ?? 0).toFixed(1)}%
                          </td>
                        </tr>
                      );
                    })}
              ...
              <p className="text-[10px] text-gray-500 font-mono mt-3">
                Sortino = annualized downside-deviation-adjusted return. Calmar = CAGR ÷ Max Drawdown. MaxDD = peak-to-trough equity curve decline.
              </p>
            </div>
          )}
