...
import { calcBlackScholesPrice } from './quantEngine';
...
export function buildAndValidateSyntheticChain(
  ...
  strikes.forEach(strike => {
    // Exact Black-Scholes pricing (replaces ATM approximation)
    const callTheo = Math.max(0.01, calcBlackScholesPrice(spotPrice, strike, T, r, iv, true));
    const putTheo = Math.max(0.01, calcBlackScholesPrice(spotPrice, strike, T, r, iv, false));
