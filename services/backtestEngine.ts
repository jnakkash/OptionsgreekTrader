...
export interface StrategyBacktestResult {
  ...
  maxDrawdownPct: number; // maximum peak-to-trough drawdown
...
function runPurgedWalkForwardBacktest(
  ...
  // ATR for MACD signal strength normalization (price-scale independent)
  ...
  const equityCurve: number[] = [100]; // normalized equity starting at 100
  ...
  for (let i = minTrainBars + embargoBars; i < candles.length; i++) {
    ...
    if (strategyType === 'EMA') {
      ...
      // Fixed: mean-reversion (buy oversold below lower band, sell at middle or above)
      const close = closes[i];
      triggerBuy = !inPosition && close < bb.lower[i];
    ...
    if (!inPosition && triggerBuy) {
      ...
      equityCurve.push(prevEquity * (1 + pnlPct / 100));
      if (pnlPct > 0) {
        wins++;
        totalProfit += pnlPct;
      } else {
  ...
  // Risk-adjusted metrics (annualizationFactor=1: per-trade returns, not daily)
  const dailyPnlsForSortino = tradePnls.map(p => p / 100);
  const sortinoRatio = calcSortinoRatio(dailyPnlsForSortino, 0, 1);
  const ddResult = calcMaxDrawdownSequence(equityCurve);
  const maxDrawdownPct = ddResult.maxDrawdownPct;
  const cagr = calcCAGR(100, equityCurve[equityCurve.length - 1], Math.max(0.1, candles.length / 252));
  const calmarRatio = calcCalmarRatio(cagr, maxDrawdownPct);
  ...
  if (strategyType === 'EMA') {
    ...
    const bandWidth = bb.upper[bb.upper.length - 1] - bb.lower[bb.lower.length - 1];
    const lastMiddle = bb.middle[bb.middle.length - 1];
    signal = lastClose > lastMiddle ? 'BULLISH' : 'BEARISH';
    // %B: 0 = at lower, 0.5 = at middle, 1 = at upper → normalize to 0-100 distance from middle
    const bbPctB = bandWidth > 0 ? (lastClose - bb.lower[bb.lower.length - 1]) / bandWidth : 0.5;
    signalStrength = Math.min(100, Math.round(Math.abs(bbPctB - 0.5) * 200));
  } else if (strategyType === 'MACD') {
    const lastHist = macdData.hist[macdData.hist.length - 1];
    signal = lastHist >= 0 ? 'BULLISH' : 'BEARISH';
    // Normalize MACD histogram relative to ATR to produce a scale-independent 0-100 score
    const lastATR = atrs[atrs.length - 1] || 1;
    signalStrength = Math.min(100, Math.round((Math.abs(lastHist) / lastATR) * 25));
  }

  const nameMap = {
    EMA: 'Multi-EMA Crossover (20/50 Trend)',
    RSI: 'RSI Momentum & Mean Reversion',
    BOLLINGER: 'Bollinger %B Mean Reversion (20/2)',
  ...
  return {
    ...
    signal,
    signalStrength,
    recentSignalDetails: `Walk-Forward: ${totalTrades} trades ${wins}W/${losses}L (${winRate.toFixed(1)}% WR, CI: ${winRateCI95.lower}-${winRateCI95.upper}%). Sortino: ${sortinoRatio.toFixed(2)}, MaxDD: ${maxDrawdownPct.toFixed(1)}%.`
  };
}
...
// 10,000-Path Monte Carlo Simulation with Fat Tails (Student-t df=5) & HAR-RV Volatility
function runMonteCarloDistributions(
  currentPrice: number,
  dailyReturns: number[],
  annualizedVolDecimal: number
): { distributions: Record<string, HorizonReturnDistribution>; terminal30dPrices: number[] } {
  const horizons = [1, 3, 5, 10, 20, 30, 60, 90, 252, 360];
  const numSimulations = 10000;
  const prng = seededRandom(42);
  const DF = 5; // Student-t degrees of freedom (heavier tails than normal)

  const meanDailyReturn = dailyReturns.reduce((a, b) => a + b, 0) / Math.max(1, dailyReturns.length);

  // HAR-RV forecast for daily vol (superior to simple historical vol)
  const harForecast = calcHARRV(dailyReturns);
  const dailyVol = harForecast.forecastDailyVol > 0 ? harForecast.forecastDailyVol : annualizedVolDecimal / Math.sqrt(252);

  const distributions: Record<string, HorizonReturnDistribution> = {};
  let terminal30dPrices: number[] = [];

  horizons.forEach(h => {
    const terminalReturns: number[] = [];
    const terminalPricesH: number[] = [];

    for (let sim = 0; sim < numSimulations; sim++) {
      let cumReturn = 0;
      for (let d = 0; d < h; d++) {
        // Proper Student-t(df=5) variate: Z/sqrt(χ²(df)/df)
        // This correctly produces fat tails (excess kurtosis = 6/(df-4) = 3 for df=5)
        const tSample = studentTVariate(prng, DF);
        const dailyDrift = meanDailyReturn - 0.5 * dailyVol * dailyVol;
        cumReturn += dailyDrift + dailyVol * tSample;
      }
      terminalReturns.push(cumReturn);
      terminalPricesH.push(currentPrice * Math.exp(cumReturn));
    }

    terminalReturns.sort((a, b) => a - b);
    terminalPricesH.sort((a, b) => a - b);

    // Store 30-day terminal prices for options strategy evaluation
    if (h === 30) terminal30dPrices = [...terminalPricesH];
    ...
    distributions[`days_${h}`] = {
      ...
      p5Target: Number(getPercentile(terminalPricesH, 0.05).toFixed(2)),
      p10Target: Number(getPercentile(terminalPricesH, 0.10).toFixed(2)),
      p25Target: Number(getPercentile(terminalPricesH, 0.25).toFixed(2)),
      p50Target: Number(getPercentile(terminalPricesH, 0.50).toFixed(2)),
      p75Target: Number(getPercentile(terminalPricesH, 0.75).toFixed(2)),
      p90Target: Number(getPercentile(terminalPricesH, 0.90).toFixed(2)),
      p95Target: Number(getPercentile(terminalPricesH, 0.95).toFixed(2)),
  ...
  return { distributions, terminal30dPrices };
}
...
export function analyzeHistoricalDataAndBacktest(
  ...
  // HAR-RV backed IV estimation with dynamic variance risk premium
  const ivEstimate = estimateImpliedVol(annualizedVolatility / 100, dailyReturns);
  const impliedVolatility = ivEstimate.iv * 100;
  const volatilityRiskPremium = ivEstimate.vrp * 100;
  ...
  // Run 10,000-Path Monte Carlo Return Distributions across 10 horizons (Student-t df=5, HAR-RV vol)
  const mcResult = runMonteCarloDistributions(currentPrice, dailyReturns, annualizedVolatility / 100);
  const distributions = mcResult.distributions;
  ...
  // Stage 2: Options Strategy Optimizer using actual MC terminal price distribution
  // Uses real simulated prices from 30-day MC paths (not a linear interpolation)
  const terminal30dPrices = mcResult.terminal30dPrices.length > 0
    ? mcResult.terminal30dPrices
    : Array.from({ length: 500 }, (_, idx) => {
        const pct = (idx + 0.5) / 500;
        const logRet = distributions['days_30'].p5ReturnPercent / 100 +
          pct * (distributions['days_30'].p95ReturnPercent / 100 - distributions['days_30'].p5ReturnPercent / 100);
