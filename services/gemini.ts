...
import { calcAnalyticalGreeks } from "./quantEngine";

// Lazy-initialized client — prevents module crash when API key is absent
let _ai: GoogleGenAI | null = null;
function getAI(): GoogleGenAI {
  if (!_ai) {
    const key = process.env.API_KEY || '';
    if (!key) throw new Error('No Gemini API key configured. Set GEMINI_API_KEY in your .env file.');
...
export const analyzeTicker = async (ticker: string): Promise<AnalysisResponse> => {
  ...
  const optionPromise = callGeminiApi<AnalysisResponse>(
    ...
    () => {
      ...
      const strike = Number((spot * 1.02).toFixed(2));
      const T = Math.max(1 / 365, targetExp.dte / 365);
      const iv = 0.28;
      const fg = calcAnalyticalGreeks(spot, strike, T, 0.05, iv, true);
      return {
        recommendation: {
          ...
          rationale: `Quantitative derivatives strategy optimized for ${ticker.toUpperCase()} at spot price $${spot.toFixed(2)}. Selected strike $${strike} with standard monthly expiration ${targetExp.dateString} (${targetExp.dte} DTE) balances Delta exposure (${fg.delta.toFixed(2)}) against Theta time decay.`,
          greeks: {
            delta: fg.delta,
            gamma: fg.gamma,
            theta: fg.theta,
